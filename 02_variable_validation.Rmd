...
d %>%
  ...
  unique()
```
Replace lang_grou with recalculated language_group
```{r}
...
Some labs used miliseconds, others seconds *Need to fix*
```{r}


#bll-manitoba has one trial with huge latency.  E-mailed lab to check
...
d %>%
  group_by(lab) %>%
  summarise(min = min(latency, na.rm = TRUE), max = max(latency, na.rm = TRUE), mean = mean(latency, na.rm=TRUE))
...
Inongruent fixation data appear to be highly left-skewed, but values are as expected
