...
##Dropping the "trial_num" random effect as "trial_num" has 0 variance: But get "model is nearly unidentifiable: very large eigenvalue - Rescale variables?" error message
#first_glmer <- d_first %>% 
#  mutate(age_days = scale(age_days, scale = FALSE)) %>%
#  filter(!is.na(first_shift)) %>%
#  mutate(first_shift = case_when(first_shift == "congruent" ~ 1, first_shift == "incongruent" ~ 0))%>%
...
##Final model: Rescaling "age_days" variable (scale = TRUE)
first_glmer2 <- d_first %>% 
