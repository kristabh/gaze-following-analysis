...
* d_first_meta: direction of first look (congruent/incongruent) and *latency of first shifts* *?akb* 
* d_freq_meta: frequency of looks to congruent vs. incongruent
* d_duration_meta: fixation duration to congruent vs. incongruent 
...
d_freq <- d %>%
  ...
  mutate(prop = (n_shift_congruent -  n_shift_incongruent)/(n_shift_congruent + n_shift_incongruent))

# Find difference per trial, then calculate sub means
d_freq_meta <- d_freq  %>%
  filter((n_shift_congruent + n_shift_incongruent) > 0) %>% # Select only trials that had a shift
  mutate(prop = (n_shift_congruent -  n_shift_incongruent)/(n_shift_congruent + n_shift_incongruent))
...
d_duration_meta <- d_duration %>% 
  filter((fixation_congruent + fixation_incongruent) > 0) %>% # Select only trials that had fixations
  mutate(prop = ())
...
d_first_meta %>%
  ...
  summarise(prop = mean(prop, na.rm = TRUE), sd = sd(prop, na.rm = TRUE))
...
freq_glmer <- d_freq %>% 
  mutate(age_days = scale(age_days, scale = FALSE)) %>%
  filter(!is.na(prop)) %>%
  glmer(prop ~ lang_group*age_days+ (1|subid) + (1|lab) + (1|trial_num),
  data = .,
  family = binomial
)
