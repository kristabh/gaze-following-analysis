...
d_freq <- d %>%
  ...
  gather(key = "object", value = "n_shifts", -iv_columns) %>%
...
d_freq_meta <- d %>%
  select(-first_shift, -latency, -fixation_congruent, - fixation_incongruent) %>%
  mutate(diff_fixation = n_shift_congruent - n_shift_incongruent) %>%
  group_by_at(vars(-trial_num, -diff_fixation, -n_shift_congruent, -n_shift_incongruent)) %>%
  summarize(diff_fixation = mean(diff_fixation, na.rm = TRUE)) # Deal with NAs at an earlier step
...
d_z_first <- d_first_meta %>%
  mutate(prop = prop - .5) %>% # Get difference from 50%
...
Model with language and age

```{r}
age_lang_meta <- metafor::rma(d ~ lang_group * age_group, vi = d_var, 
                    slab = lab, data = d_z_first, method = "REML") 
summary(age_lang_meta)
```
