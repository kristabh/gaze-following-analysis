...
ggplot(first_forest_data,
  ...
  scale_colour_manual(name = "Language group", values = c("#bdbdbd", "#636363", "#000000")) +
...
latency_lang_only_matched_in6to9 <- metafor::rma(d ~ lang_group, vi = d_var,
                                  slab = lab, data = d_z_latency_6to9, method = "REML")

d_z_latency_12to15 <- d_z_latency%>%
  filter(age_group == "12-15 mo")

latency_lang_only_matched_in12to15 <- metafor::rma(d ~ lang_group, vi = d_var,
...
ggplot(latency_forest_data,
  ...
  scale_size_continuous(guide = FALSE) +
  scale_colour_manual(name = "Language group", values = c("#bdbdbd", "#636363", "#000000")) +
  scale_y_continuous(limits=c(-1, 15), breaks=seq(-1,15,1)) +
  xlab("Lab") +
  ylab("Effect Size: Latency") +
  theme(legend.position = "bottom")
```
...
Our final model specification for first shift was:
...
\noindent Table 4 shows coefficient estimates from this model and Figure 8 visualizes this model. The significant effect of age indicated that the duration of fixations was greater for older infants. The main effect of aoi was also significant, suggesting that infants looked longer at the congruent aoi than at the incongruent aoi. While the effect of bilingualism was significant, it was suggested that in general bilingual infants looked longer at the AOIs than did the monolingual infants. No significant interactions were observed.
...
Overall, our mixed-effects regression revealed that early gaze-following development is significantly modulated by age-related changes, where older infants tend to show a more reliable gaze-following ability than younger infants. In particular, older infants were shown to be more accurate and rapid than younger infants in directing their first gaze towards the congruent objects. Older infants also looked longer and more frequent at the congruent objects than at the incongruent objects. On the contrary, the effect of bilingualism did not predicted infants’ gaze-following latency and accuracy, but predicted only their duration of fixations. In general, relative to monolingual infants, bilingual infants looked longer at both the congruent and incongruent objects. Together, these findings suggested that monolingual and bilingual infants appear to follow a similar trajectory of gaze-following development despite the difference in their language experience. 
