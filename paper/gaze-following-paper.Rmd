...
We began by examining the relation of the proportion of first looks to bilingualism and age, fitting the main effect model to the `r nrow(d_z_first)` separate group means and variances (after aggregating by lab, age, and language group). The meta-analysis on first look yielded a reference-level intercept of `r round(first_meta$b[1],2)` (CI = [`r round(first_meta$ci.lb[1],2)` - `r round(first_meta$ci.ub[1], 2)`], $z = `r round(first_meta$z[1], 2)`$, $p = `r papaja::printp(first_meta$pval[1])`$), reflecting the mean effect size for 6–9 month-old monolingual infants. Age yielded an effect of `r round(first_meta$b[3],2)` (CI = [`r round(first_meta$ci.lb[3],2)` - `r round(first_meta$ci.ub[3], 2)`], $z = `r round(first_meta$z[3], 2)`$, $p = `r papaja::printp(first_meta$pval[3])`$), indicating a non-significant increase in the average proportion of first looks for older 12-15 month-old infants. Bilingualism yielded an effect of `r round(first_meta$b[2],2)` (CI = [`r round(first_meta$ci.lb[2],2)` - `r round(first_meta$ci.ub[2], 2)`], $z = `r round(first_meta$z[2], 2)`$, $p = `r papaja::printp(first_meta$pval[2])`$), indicating that no significant evidence was found for bilingualism as a moderator of differences in proportion of first looks. Moreover, the interaction between bilingualism and age yielded an effect of `r round(first_meta$b[4],2)` (CI = [`r round(first_meta$ci.lb[4],2)` - `r round(first_meta$ci.ub[4], 2)`], $z = `r round(first_meta$z[4], 2)`$, $p = `r papaja::printp(first_meta$pval[4])`$), suggesting that there was not a reliable difference in terms of bilingualism between the older and younger infants. Overall, there was no evidence that bilingual infants would perform differently from monolingual infants, and the effect of bilingualism also did not interact with the effect of age. A forest plot for this meta-analysis is shown in Figure 2. 
...
We then investigated the relation of frequency of looks to bilingualism and age group. The reference-level intercept (i.e., 6–9 month-old monolingual infants) was `r round(freq_meta$b[1],2)` (CI = [`r round(freq_meta$ci.lb[1],2)` - `r round(freq_meta$ci.ub[1], 2)`], $z = `r round(freq_meta$z[1], 2)`$, $p = `r papaja::printp(freq_meta$pval[1])`$). Age yielded an effect of `r round(freq_meta$b[3],2)` (CI = [`r round(freq_meta$ci.lb[3],2)` - `r round(freq_meta$ci.ub[3], 2)`], $z = `r round(freq_meta$z[3], 2)`$, $p = `r papaja::printp(freq_meta$pval[3])`$), indicating that 12-15 month-old infants did not significantly look more frequently at the AOIs. As for bilingualism, an effect of `r round(freq_meta$b[2],2)` (CI = [`r round(freq_meta$ci.lb[2],2)` - `r round(freq_meta$ci.ub[2], 2)`], $z = `r round(freq_meta$z[2], 2)`$, $p = `r papaja::printp(freq_meta$pval[2])`$) was yielded, suggesting that bilingual infants did not perform differently from the monolingual infants. The interaction between bilingualism and age yielded an effect of `r round(freq_meta$b[4],2)` (CI = [`r round(freq_meta$ci.lb[4],2)` - `r round(freq_meta$ci.ub[4], 2)`], $z = `r round(freq_meta$z[4], 2)`$, $p = `r papaja::printp(freq_meta$pval[4])`$), implying that no differences were observed between monolingual and bilingual infants at both ages. A forest plot for this meta-analysis is shown in Figure 3. 
...
The cross-lab meta-analysis on duration of looks yielded an estimated reference-level intercept (i.e., 6–9 month-old monolingual infants) of `r round(dur_meta$b[1],2)` (CI = [`r round(dur_meta$ci.lb[1],2)` - `r round(dur_meta$ci.ub[1], 2)`], $z = `r round(dur_meta$z[1], 2)`$, $p = `r papaja::printp(dur_meta$pval[1])`$). Age yielded an effect of `r round(dur_meta$b[3],2)` (CI = [`r round(dur_meta$ci.lb[3],2)` - `r round(dur_meta$ci.ub[3], 2)`], $z = `r round(dur_meta$z[3], 2)`$, $p = `r papaja::printp(dur_meta$pval[3])`$), indicating 12-15 month-old infants performed similarly as 6-9 month-old infants. The effect of bilingualism was `r round(dur_meta$b[2],2)` (CI = [`r round(dur_meta$ci.lb[2],2)` - `r round(dur_meta$ci.ub[2], 2)`], $z = `r round(dur_meta$z[2], 2)`$, $p = `r papaja::printp(dur_meta$pval[2])`$), suggesting that bilingual infants did not look significantly longer compared to monolingual infants. As for the interaction between bilingualism and age, the effect was `r round(dur_meta$b[4],2)` (CI = [`r round(dur_meta$ci.lb[4],2)` - `r round(dur_meta$ci.ub[4], 2)`], $z = `r round(dur_meta$z[4], 2)`$, $p = `r papaja::printp(dur_meta$pval[4])`$), implying that there was no evidence of differences between monolingual and bilingual infants across both ages. A forest plot for this meta-analysis is shown in Figure 4. 
...
The cross-lab meta-analysis on latency yielded a reference-level intercept (i.e., 6–9 month-old monolingual infants) of `r round(Latency_meta$b[1],2)` (CI = [`r round(Latency_meta$ci.lb[1],2)` - `r round(Latency_meta$ci.ub[1], 2)`], $z = `r round(Latency_meta$z[1], 2)`$, $p = `r papaja::printp(Latency_meta$pval[1])`$). Age yielded an effect of `r round(Latency_meta$b[3],2)` (CI = [`r round(Latency_meta$ci.lb[3],2)` - `r round(Latency_meta$ci.ub[3], 2)`], $z = `r round(Latency_meta$z[3], 2)`$, $p = `r papaja::printp(Latency_meta$pval[3])`$), indicating that 12-15 month-old infants was not significantly more rapid than 6-9 month-old infants in terms of their reaction times. Bilingualism yielded an effect of `r round(Latency_meta$b[2],2)` (CI = [`r round(Latency_meta$ci.lb[2],2)` - `r round(Latency_meta$ci.ub[2], 2)`], $z = `r round(Latency_meta$z[2], 2)`$, $p = `r papaja::printp(Latency_meta$pval[2])`$), suggesting no differences in reaction times between bilingual and monolingual infants. The effect of the interaction between bilingualism and age was `r round(Latency_meta$b[4],2)` (CI = [`r round(Latency_meta$ci.lb[4],2)` - `r round(Latency_meta$ci.ub[4], 2)`], $z = `r round(Latency_meta$z[4], 2)`$, $p = `r papaja::printp(Latency_meta$pval[4])`$), implying that the difference between monolingual and bilingual infants was not significant at both ages. A forest plot for this meta-analysis is shown in Figure 5. 
...
Overall, our meta-analysis models revealed greater effect sizes in the measures of first looks and frequency of looks, such that these two measure appeared to be more sensitive measures than duration of fixations or latency. Moreover, these two first measures revealed that the effect of age had greater effect sizes, suggesting that older infants showed more reliable gaze-following than younger infants. However, we observed no evidence of bilingualism as a statistically significant moderator of gaze-following development, suggesting that bilingual infants tend to perform similarly as the monolingual infants.
