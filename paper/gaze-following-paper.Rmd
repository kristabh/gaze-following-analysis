...
We began by examining the relation of the proportion of first looks to bilingualism and age, fitting the main effect model to the `r nrow(d_z_first)` separate group means and variances (after aggregating by lab, age, and language group). The meta-analysis on first look yielded a reference-level intercept of `r round(first_meta$b[1],2)` (CI = [`r round(first_meta$ci.lb[1],2)` - `r round(first_meta$ci.ub[1], 2)`], $z = `r round(first_meta$z[1], 2)`$, $p `r papaja::printp(first_meta$pval[1])`$), reflecting the mean effect size for 6–9 month-old monolingual infants. Bilingualism yielded an effect of `r round(first_meta$b[2],2)` (CI = [`r round(first_meta$ci.lb[2],2)` - `r round(first_meta$ci.ub[2], 2)`], $z = `r round(first_meta$z[2], 2)`$, $p `r papaja::printp(first_meta$pval[2])`$), indicating a non-significant increase in measured effect size for bilingual relative to monolingual infants. Moreover, age yielded an effect of `r round(first_meta$b[3],2)` (CI = [`r round(first_meta$ci.lb[3],2)` - `r round(first_meta$ci.ub[3], 2)`], $z = `r round(first_meta$z[3], 2)`$, $p `r papaja::printp(first_meta$pval[3])`$), indicating a non-significant increase in the average proportion of first looks for older 12-15 month-old infants. Moreover, the interaction between bilingualism and age yielded an effect of `r round(first_meta$b[4],2)` (CI = [`r round(first_meta$ci.lb[4],2)` - `r round(first_meta$ci.ub[4], 2)`], $z = `r round(first_meta$z[4], 2)`$, $p `r papaja::printp(first_meta$pval[4])`$), suggesting that there was not a reliable difference in terms of bilingualism between the older and younger infants. Therefore, no significant differeces in the proportion of first looks were observed among monolingual and bilingual infants as well as older and young infants. A pirate plot for this meta-analysis is shown in Figure 2. 

```{r fig2, fig.cap="Pirate plot for the cross-lab meta-analysis on the proportion of first look."}
...
We then investigated the relation of frequency of looks to bilingualism and age group. The reference-level intercept (i.e., 6–9 month-old monolingual infants) was `r round(freq_meta$b[1],2)` (CI = [`r round(freq_meta$ci.lb[1],2)` - `r round(freq_meta$ci.ub[1], 2)`], $z = `r round(freq_meta$z[1], 2)`$, $p `r papaja::printp(freq_meta$pval[1])`$). However, no significant effect was yielded in this meta-analysis. Bilingualism yielded an effect of `r round(freq_meta$b[2],2)` (CI = [`r round(freq_meta$ci.lb[2],2)` - `r round(freq_meta$ci.ub[2], 2)`], $z = `r round(freq_meta$z[2], 2)`$, $p `r papaja::printp(freq_meta$pval[2])`$), suggesting that the frequency of looks was on average not significantly larger for bilingual infants. As for age, an effect of  `r round(freq_meta$b[3],2)` (CI = [`r round(freq_meta$ci.lb[3],2)` - `r round(freq_meta$ci.ub[3], 2)`], $z = `r round(freq_meta$z[3], 2)`$, $p `r papaja::printp(freq_meta$pval[3])`$) was yielded, indicating a non-significant increase in frequency of looks for 12-15 month-old infants. The interaction between bilingualism and age yielded an effect of `r round(freq_meta$b[4],2)` (CI = [`r round(freq_meta$ci.lb[4],2)` - `r round(freq_meta$ci.ub[4], 2)`], $z = `r round(freq_meta$z[4], 2)`$, $p `r papaja::printp(freq_meta$pval[4])`$), implying that the difference between monolingual and bilingualism infants was not significantly lower for 12-15 month-old infants than for 6-9 month-old. A pirate plot for this meta-analysis is shown in Figure 3. 

```{r fig3, fig.cap="Pirate plot for the cross-lab meta-analysis on frequency of looks."}
...
The cross-lab meta-analysis on duration of looks yielded an estimated reference-level intercept (i.e., 6–9 month-old monolingual infants) of `r round(dur_meta$b[1],2)` (CI = [`r round(dur_meta$ci.lb[1],2)` - `r round(dur_meta$ci.ub[1], 2)`], $z = `r round(dur_meta$z[1], 2)`$, $p `r papaja::printp(dur_meta$pval[1])`$). Similiar to the two previous meta-analyses, no significant effect was yielded. The effect of bilingualism was `r round(dur_meta$b[2],2)` (CI = [`r round(dur_meta$ci.lb[2],2)` - `r round(dur_meta$ci.ub[2], 2)`], $z = `r round(dur_meta$z[2], 2)`$, $p `r papaja::printp(dur_meta$pval[2])`$), suggesting a slight but non-significant increase in duration of looks for biligual infants compared to monolingual infants. Age yielded an effect of `r round(dur_meta$b[3],2)` (CI = [`r round(dur_meta$ci.lb[3],2)` - `r round(dur_meta$ci.ub[3], 2)`], $z = `r round(dur_meta$z[3], 2)`$, $p `r papaja::printp(dur_meta$pval[3])`$), indicating a non-significant increase in measured effect size for 12-15 month-old infants relative to 6-9 month-old infants. As for the interaction between bilingualism and age, the effect was `r round(dur_meta$b[4],2)` (CI = [`r round(dur_meta$ci.lb[4],2)` - `r round(dur_meta$ci.ub[4], 2)`], $z = `r round(dur_meta$z[4], 2)`$, $p `r papaja::printp(dur_meta$pval[4])`$), implying that the difference between monolingual and bilingualism infants was not significantly higher for 12-15 month-old infants than for 6-9 month-old. A pirate plot for this meta-analysis is shown in Figure 4. 

```{r fig4, fig.cap="Pirate plot for the cross-lab meta-analysis on duration of looks."}
...
The cross-lab meta-analysis on latency yielded a reference-level intercept (i.e., 6–9 month-old monolingual infants) of `r round(Latency_meta$b[1],2)` (CI = [`r round(Latency_meta$ci.lb[1],2)` - `r round(Latency_meta$ci.ub[1], 2)`], $z = `r round(Latency_meta$z[1], 2)`$, $p `r papaja::printp(Latency_meta$pval[1])`$). Bilingualism yielded an effect of `r round(Latency_meta$b[2],2)` (CI = [`r round(Latency_meta$ci.lb[2],2)` - `r round(Latency_meta$ci.ub[2], 2)`], $z = `r round(Latency_meta$z[2], 2)`$, $p `r papaja::printp(Latency_meta$pval[2])`$), suggesting a non-significant increase in duration of looks for biligual infants relative to monolingual infants. Age yielded an effect of `r round(Latency_meta$b[3],2)` (CI = [`r round(Latency_meta$ci.lb[3],2)` - `r round(Latency_meta$ci.ub[3], 2)`], $z = `r round(Latency_meta$z[3], 2)`$, $p `r papaja::printp(Latency_meta$pval[3])`$), indicating a non-significant decrease in latency for 12-15 month-old infants compared to 6-9 month-old infants. The effect of the interaction between bilingualism and age was `r round(Latency_meta$b[4],2)` (CI = [`r round(Latency_meta$ci.lb[4],2)` - `r round(Latency_meta$ci.ub[4], 2)`], $z = `r round(Latency_meta$z[4], 2)`$, $p `r papaja::printp(Latency_meta$pval[4])`$), implying that the difference between monolingual and bilingualism infants was smaller for 12-15 month-old infants than for 6-9 month-old but this difference was not significant. A pirate plot for this meta-analysis is shown in Figure 5. 

```{r fig5, fig.cap="Pirate plot for the cross-lab meta-analysis on latency."}
...
We ran separate models for each DV. We fit all models using the `lme4` package [@bates2014fitting]. For first_shift, we fit a logistic model as this variable is binary at the trial level. The initial model specification was:
...
After pruning random effects for non-convergence, our final model specification for first shift was:

\begin{equation}
\begin{split}
\text{first_shift} \sim & \text{bilingual} * \text{age_days} + \\
& (1 \mid \text{subid}) + \\
& (1 \mid \text{lab}).
\end{split}
\end{equation}
...
rownames(firstshift_coefs) <- c("Intercept", "Bilingualism", "Age_days", 
                     "Bilingualism * Age_days") 
...
\noindent Table 2 shows coefficient estimates from this model and Figure 6 visualizes this model. To aid convergence, we scaled age, and the random effect of item was removed. Positive coefficients indicate a higher probability of making first look to the congruent object. While there is no obvious evidence for a difference between monolingual and bilingual infants, a significant positive effect of age was found, suggesting that older infants had a greater probability of making their first looks to the congruent as opposed to the incongruent object. The interaction of bilingualism and age was, however, not significant.

```{r fig6, fig.cap="The logistic regression model predicting the probability of making first look to the congruent object."}
...
d_first%>%
  ...
  ggplot() +
  aes(x=age_days, y=first_shift, color=lang_group) +
  geom_point()+
...
The linear mixed-effects model for frequency of shift was also modified by pruning random effects for non-convergence. The final model specification for frequency of shift was:

\begin{equation}
\begin{split}
\text{freq_shift} \sim & \text{bilingual} * \text{age_days} *  \text{aoi} + \\
& (1 \mid \text{subid}) + \\
& (1 \mid \text{item}).
\end{split}
\end{equation}
...
rownames(freq_coefs) <- c("Intercept", "Bilingualism", "Age_days", "AOI",
                     "Bilingualism * Age_days", "Bilingualism * AOI", "Age_days * AOI",
                     "Bilingualism * Age_days * AOI") 
...
\noindent Table 3 shows coefficient estimates from this model and Figure 7 visualizes this model. The random effect of lab was removed to aid convergence. There were significant main effects of age and aoi, where older infants and the congruent aoi had greater amount of frequency as opposed to younger infants and incongruent aoi, respectively. The interaction of age and aoi was also significant, further suggesting that older infants shifted more frequently to the congruent than the incongruent AOI compared to younger infants. The effect of bilingualism, however, was not significant, neither were its 2-way and 3-way interactions with age and aoi; this suggests that there was not a reliable difference between bilingual and monolingual infants in the number of times they shifted gaze towards the congruent aoi. 

```{r fig7, fig.cap="The linear regression model predicting the frequency of shift towards the AOI."}
...
d_freq %>%
  ...
  geom_smooth(method = "lm", aes(fill=object)) +
  ylab("Frequency of shifts towards the AOI") +
  xlab("Age in days") +
  scale_colour_manual(name = "AOI", values = c("darkcyan", "darkred")) +
  scale_fill_manual(name = "AOI", values = c("darkcyan", "darkred")) + 
  theme(text = element_text(size = 12), legend.position = "bottom")
```
...
\begin{equation}
\begin{split}
\text{total_look} \sim & \text{bilingual} * \text{age_days} *  \text{aoi} + \\
& (1 \mid \text{lab}) + \\
& (1 \mid \text{item}).
\end{split}
\end{equation}
...
rownames(duration_coefs) <- c("Intercept", "Bilingualism", "Age_days", "AOI",
                     "Bilingualism * Age_days", "Bilingualism * AOI", "Age_days * AOI",
                     "Bilingualism * Age_days * AOI") 
...
\noindent Table 4 shows coefficient estimates from this model and Figure 8 visualizes this model. The random effect of subject was removed to aid convergence. The significant effect of age indicated that the duration of fixations was greater for older infants. The main effect of aoi was also significant; with congruent aoi as the reference level, it is suggested that congruent aoi had longer duration of fixations compared to incongruent aoi. While the effect of bilingualism was non-significant, it is not a strong predictor of duration of fixations in our data. Other interactions were also non-significant.

```{r fig8, fig.cap="The linear regression model predicting duration of fixations towards the AOI."}
...
As the preregistered model specification for latency ran into convergence issue, random effects were pruned for the final model specification:

\begin{equation}
\begin{split}
\text{latency} \sim & \text{bilingual} * \text{age_days} * \text{aoi} + \\
& (1 \mid \text{subid}) + \\
& (1 \mid \text{lab}) + \\
& (1 \mid \text{item}).
\end{split}
...
rownames(latency_coefs) <- c("Intercept", "Bilingualism", "Age_days", "AOI",
                     "Bilingualism * Age_days", "Bilingualism * AOI", "Age_days * AOI",
                     "Bilingualism * Age_days * AOI") 
...
\noindent Table 5 shows coefficient estimates from this model and Figure 9 visualizes this model. The main effect of age was significant, suggesting that older infants had a shorter latency to first fixation than younger infants. However, the effect of aoi was not significant, implying that latency did not reliably differ as a function of whether it is to the congruent or incongruent AOI. Other coefficients were also non-significant.

```{r fig9, fig.cap="The linear regression model predicting duration of fixations towards the AOI."}
