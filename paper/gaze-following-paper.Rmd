...
We then investigated the relation of frequency of looks to bilingualism and age group. The reference-level intercept (i.e., 6–9 month-old monolingual infants) was `r round(freq_meta$b[1],2)` (CI = [`r round(freq_meta$ci.lb[1],2)` - `r round(freq_meta$ci.ub[1], 2)`], $z = `r round(freq_meta$z[1], 2)`$, $p `r papaja::printp(freq_meta$pval[1])`$). However, no significant effect was yielded in this meta-analysis. Bilingualism yielded an effect of `r round(freq_meta$b[2],2)` (CI = [`r round(freq_meta$ci.lb[2],2)` - `r round(freq_meta$ci.ub[2], 2)`], $z = `r round(freq_meta$z[2], 2)`$, $p `r papaja::printp(freq_meta$pval[2])`$), suggesting that the frequency of looks was on average not significantly larger for bilingual infants. As for age, an effect of  `r round(freq_meta$b[3],2)` (CI = [`r round(freq_meta$ci.lb[3],2)` - `r round(freq_meta$ci.ub[3], 2)`], $z = `r round(freq_meta$z[3], 2)`$, $p `r papaja::printp(freq_meta$pval[3])`$) was yielded, indicating a non-significant increase in frequency of looks for 12-15 month-old infants. The interaction between bilingualism and age yielded an effect of `r round(freq_meta$b[4],2)` (CI = [`r round(freq_meta$ci.lb[4],2)` - `r round(freq_meta$ci.ub[4], 2)`], $z = `r round(freq_meta$z[4], 2)`$, $p `r papaja::printp(freq_meta$pval[4])`$), implying that the difference between monolingual and bilingualism infants was not significantly lower for 12-15 month-old infants than for 6-9 month-old. A forest plot for this meta-analysis is shown in Figure 3. 
...
The cross-lab meta-analysis on duration of looks yielded an estimated reference-level intercept (i.e., 6–9 month-old monolingual infants) of `r round(dur_meta$b[1],2)` (CI = [`r round(dur_meta$ci.lb[1],2)` - `r round(dur_meta$ci.ub[1], 2)`], $z = `r round(dur_meta$z[1], 2)`$, $p `r papaja::printp(dur_meta$pval[1])`$). Similiar to the two previous meta-analyses, no significant effect was yielded. The effect of bilingualism was `r round(dur_meta$b[2],2)` (CI = [`r round(dur_meta$ci.lb[2],2)` - `r round(dur_meta$ci.ub[2], 2)`], $z = `r round(dur_meta$z[2], 2)`$, $p `r papaja::printp(dur_meta$pval[2])`$), suggesting a slight but non-significant increase in duration of looks for biligual infants compared to monolingual infants. Age yielded an effect of `r round(dur_meta$b[3],2)` (CI = [`r round(dur_meta$ci.lb[3],2)` - `r round(dur_meta$ci.ub[3], 2)`], $z = `r round(dur_meta$z[3], 2)`$, $p `r papaja::printp(dur_meta$pval[3])`$), indicating a non-significant increase in measured effect size for 12-15 month-old infants relative to 6-9 month-old infants. As for the interaction between bilingualism and age, the effect was `r round(dur_meta$b[4],2)` (CI = [`r round(dur_meta$ci.lb[4],2)` - `r round(dur_meta$ci.ub[4], 2)`], $z = `r round(dur_meta$z[4], 2)`$, $p `r papaja::printp(dur_meta$pval[4])`$), implying that the difference between monolingual and bilingualism infants was not significantly higher for 12-15 month-old infants than for 6-9 month-old. A forest plot for this meta-analysis is shown in Figure 4. 
...
The cross-lab meta-analysis on latency yielded a reference-level intercept (i.e., 6–9 month-old monolingual infants) of `r round(Latency_meta$b[1],2)` (CI = [`r round(Latency_meta$ci.lb[1],2)` - `r round(Latency_meta$ci.ub[1], 2)`], $z = `r round(Latency_meta$z[1], 2)`$, $p `r papaja::printp(Latency_meta$pval[1])`$). Bilingualism yielded an effect of `r round(Latency_meta$b[2],2)` (CI = [`r round(Latency_meta$ci.lb[2],2)` - `r round(Latency_meta$ci.ub[2], 2)`], $z = `r round(Latency_meta$z[2], 2)`$, $p `r papaja::printp(Latency_meta$pval[2])`$), suggesting a non-significant increase in duration of looks for biligual infants relative to monolingual infants. Age yielded an effect of `r round(Latency_meta$b[3],2)` (CI = [`r round(Latency_meta$ci.lb[3],2)` - `r round(Latency_meta$ci.ub[3], 2)`], $z = `r round(Latency_meta$z[3], 2)`$, $p `r papaja::printp(Latency_meta$pval[3])`$), indicating a non-significant decrease in latency for 12-15 month-old infants compared to 6-9 month-old infants. The effect of the interaction between bilingualism and age was `r round(Latency_meta$b[4],2)` (CI = [`r round(Latency_meta$ci.lb[4],2)` - `r round(Latency_meta$ci.ub[4], 2)`], $z = `r round(Latency_meta$z[4], 2)`$, $p `r papaja::printp(Latency_meta$pval[4])`$), implying that the difference between monolingual and bilingualism infants was smaller for 12-15 month-old infants than for 6-9 month-old but this difference was not significant. A forest plot for this meta-analysis is shown in Figure 5. 
...
```{r fig6, fig.cap="The logistic regression model predicting the probability of making first look to the congruent object, plotted with individual participants' probabilities."}
...
```{r fig9, fig.cap="The linear regression model predicting latency of fixations towards the AOI."}
