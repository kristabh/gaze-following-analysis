...
author: 
  ...
  - name          : "Ágnes Melinda Kovács"
    affiliation   : "10"
  - name          : "Alexandra Laoun-Rubenstein"
    affiliation   : "1"
...
author_note: |
  Individual participating labs acknowledge funding support from: TO BE ADDED
...
The cross-lab meta-analysis on latency towards the congruent object yielded a reference-level mean latency estimate of `r round(Latency_meta$b[1],2)` (CI = [`r round(Latency_meta$ci.lb[1],2)` - `r round(Latency_meta$ci.ub[1], 2)`], $z = `r round(Latency_meta$z[1], 2)`$, $p = `r papaja::printp(Latency_meta$pval[1])`$) for 6–9 month-old monolingual infants. With the effect of age, the mean latency estimate decreased significantly by `r round(Latency_meta$b[3],2)` (CI = [`r round(Latency_meta$ci.lb[3],2)` - `r round(Latency_meta$ci.ub[3], 2)`], $z = `r round(Latency_meta$z[3], 2)`$, $p = `r papaja::printp(Latency_meta$pval[3])`$); in other words, 12–15 month-old monolingual infants were faster than 6–9 month-old monolingual infants to fixate the congruent object. The effect of bilingualism increased the mean latency estimate by `r round(Latency_meta$b[2],2)` (CI = [`r round(Latency_meta$ci.lb[2],2)` - `r round(Latency_meta$ci.ub[2], 2)`], $z = `r round(Latency_meta$z[2], 2)`$, $p = `r papaja::printp(Latency_meta$pval[2])`$); in other words, 6–9 month-old bilingual infants were (non-significantly) slower than monolingual infants to fixate the congruent object. The interaction between bilingualism and age suggested a possible attenuation of this pattern for older bilingual versus monolingual infants, although this did not reach statistical significance (estimate = `r round(Latency_meta$b[4],2)`, CI = [`r round(Latency_meta$ci.lb[4],2)` - `r round(Latency_meta$ci.ub[4], 2)`], $z = `r round(Latency_meta$z[4], 2)`$, $p = `r papaja::printp(Latency_meta$pval[4])`$). Pairwise comparisons revealed that at 6–9 months the bilinguals tended to be slower than the monolinguals by `r round(pairwise$Lb[1],2)` ($se = `r round(pairwise$se[1],2)`$, $z = `r round(pairwise$z[1], 2)`$, $p = `r papaja::printp(pairwise$pval[1])`$). However, at the age of 12–15 months, there was no longer any evidence of a difference in target fixation latency between monolingual and bilingual infants (estimate = `r round(pairwise$Lb[2],2)`, $se = `r round(pairwise$se[2],2)`$, $z = `r round(pairwise$z[2], 2)`$, $p = `r papaja::printp(pairwise$pval[2])`$). A forest plot for this meta-analysis is shown in Figure 5.
...
##Plotting the age_days*object interaction
#emmeans(latency_lmer, pairwise ~ object|age_days, at = list(age_days = c(-2:1)))
#library(interactions)
#interact_plot(latency_lmer, pred = age_days, modx = object)
...
#    Contribution == "analysis" ~ "Data management and analysis",
