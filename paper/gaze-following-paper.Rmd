...
The cross-lab meta-analysis on latency towards the congruent object yielded an overall mean latency of `r round(Latency_meta$b[1],2)` (CI = [`r round(Latency_meta$ci.lb[1],2)` - `r round(Latency_meta$ci.ub[1], 2)`], $z = `r round(Latency_meta$z[1], 2)`$, $p = `r papaja::printp(Latency_meta$pval[1])`$) for 6–9 month-old monolingual infants. With the effect of age, the mean latency decreased significantly by `r round(Latency_meta$b[3],2)` (CI = [`r round(Latency_meta$ci.lb[3],2)` - `r round(Latency_meta$ci.ub[3], 2)`], $z = `r round(Latency_meta$z[3], 2)`$, $p = `r papaja::printp(Latency_meta$pval[3])`$); in other words, 12–15 month-old infants were faster than 6–9 month-old infants to fixate the congruent object. The effect of bilingualism increased the mean latency by `r round(Latency_meta$b[2],2)` (CI = [`r round(Latency_meta$ci.lb[2],2)` - `r round(Latency_meta$ci.ub[2], 2)`], $z = `r round(Latency_meta$z[2], 2)`$, $p = `r papaja::printp(Latency_meta$pval[2])`$); in other words, bilingual infants were (non-significantly) slower than monolingual infants to fixate the congruent object. The interaction between bilingualism and age suggested a possible attenuation of this pattern for older bilingual vs. monolingual infants, although this did not reach statistical significance (estimate = `r round(Latency_meta$b[4],2)`, CI = [`r round(Latency_meta$ci.lb[4],2)` - `r round(Latency_meta$ci.ub[4], 2)`], $z = `r round(Latency_meta$z[4], 2)`$, $p = `r papaja::printp(Latency_meta$pval[4])`$). Pairwise comparisons revealed that at 6–9 months the bilinguals tended to be slower than the monolinguals by `r round(pairwise$Lb[1],2)` ($se = `r round(pairwise$se[1],2)`$, $z = `r round(pairwise$z[1], 2)`$, $p = `r papaja::printp(pairwise$pval[1])`$). However, at the age of 12–15 months, there was no longer evidence of a difference in target fixation latency between monolingual and bilingual infants (estimate = `r round(pairwise$Lb[2],2)`, $se = `r round(pairwise$se[2],2)`$, $z = `r round(pairwise$z[2], 2)`$, $p = `r papaja::printp(pairwise$pval[2])`$). A forest plot for this meta-analysis is shown in Figure 5.
...
\noindent Table 4 shows coefficient estimates from this model and Figure 8 visualizes this model. The significant effect of age indicated that in general older infants looked longer at the objects than did the younger infants. The main effect of aoi was also significant, suggesting that infants looked longer at the congruent object than at the incongruent object. The effect of bilingualism, however, was not significant, suggesting that overall no reliable difference was found between bilingual and monolingual infants in terms of their duration of looking at the objects.　No significant interactions were observed.
...
ManyBabies Consortium. (2019a; In press). Quantifying sources of variability in infancy research using the infant-directed speech preference. Advances in Methods and Practices in Psychological Science.

ManyBabies Consortium. (2019b; Registered Report accepted under Stage 2 preparation.). A multi-lab study of bilingual infants: Exploring the preference for infant-directed speech. Advances in Methods and Practices in Psychological Science.
