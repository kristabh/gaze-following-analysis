...
author: 
  - name          : "Krista Byers-Heinlein"
    affiliation   : "1"
    corresponding : yes    # Must define only one corresponding author. Work-around to get two corresponding authors
    address       : "Department of Psychology, 7141 Sherbrooke St. West, Montreal, QC, Canada, H2T1V2, or Leher Singh, Department of Psychology, 7 Arts Link, AS5 04-30, Singapore 117571"
    email         : "k.byers@concordia.ca or leher.singh@nus.edu.sg"
...
```{r fig1, fig.cap="Screenshot of one of the videos presented to infants showing the three areas of interest (AOIs) used: face, congruent object, and incongruent object.", fig.align='center'}
...
```{r fig2, fig.cap="Forest plot for the cross-lab meta-analysis on the proportion of first look.", echo = FALSE, dpi = 1000}
...
```{r frequency-of-looks, echo = FALSE}
...
d_freq_meta <- d  %>%
  ...
  summarise(prop = mean(prop, na.rm = TRUE)) # Average proportion difference score by baby
...
```{r fig3, fig.cap="Forest plot for the cross-lab meta-analysis on frequency of looks.", echo = FALSE, dpi = 1000}
...
```{r duration-of-looks, echo = FALSE}
...
d_duration_meta <- d_duration %>%
  pivot_wider(names_from = object, values_from = duration) %>%
  mutate(prop = (congruent - incongruent)/(congruent + incongruent)) %>% # Calculate proportion difference score
  filter(!is.na(prop)) %>%
  group_by(subid, age_group, lang_group, lab) %>%
  summarise(prop = mean(prop, na.rm = TRUE)) # Average proportion difference score by baby
...
d_latency_meta <- d_latency %>%
  mutate(object = as.factor(object)) %>%
  filter(object != "incongruent") %>%
  filter(!is.na(latency)) %>%
  group_by(subid, age_group, lang_group, lab) %>%
  summarise(latency = mean(latency, na.rm = TRUE)) # Average latency by baby
...
```{r fig5, fig.cap="Forest plot for the cross-lab meta-analysis on latency.", echo = FALSE, dpi = 1000}
...
```{r coef_table_firstshift, results="asis", echo = FALSE}

first_glmer <- d_first %>% 
  mutate(age_days = scale(age_days, scale = TRUE)) %>%
  filter(!is.na(first_shift)) %>%
  mutate(first_shift = fct_rev(first_shift)) %>% # So higher numbers = more looks to congruent
...
```{r fig6, fig.cap="The logistic regression model predicting the probability of making first look to the congruent object, plotted with individual participants' probabilities.", fig.width = 5, echo = FALSE, dpi = 1000}
...
```{r coef_table_frequency, results="asis", echo = FALSE}
freq_lmer <- d_freq %>% 
  mutate(age_days = scale(age_days, scale = TRUE)) %>%
...
```{r fig7, fig.cap="The linear regression model predicting the frequency of shift towards the AOI, with error bars showing 95% confidence interval.", echo = FALSE, dpi = 1000}
...
```{r coef_table_duration, results="asis", echo = FALSE}

duration_lmer <- d_duration %>% 
  mutate(age_days = scale(age_days, scale = TRUE)) %>%
  lmer(duration ~ lang_group*age_days*object + (1|subid) + (1|lab) + (1|trial_num),
  data = .)
...
```{r fig8, fig.cap="The linear regression model predicting duration of fixations towards the AOI, with error bars showing 95% confidence interval.", echo = FALSE, dpi = 1000}
...
```{r coef_table_latency, results="asis", echo = FALSE}
...
```{r fig9, fig.cap="The linear regression model predicting latency of fixations towards the AOI, with error bars showing 95% confidence interval.", echo = FALSE, dpi = 1000}
