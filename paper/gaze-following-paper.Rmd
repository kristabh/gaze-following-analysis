...
d_latency_meta <- d_latency %>%
  ...
  mutate(latency = mean(latency)) %>% # Average latency by baby
...
The cross-lab meta-analysis on latency towards the congruent object yielded a reference-level mean log latency estimate of `r round(Latency_meta$b[1],2)` (CI = [`r round(Latency_meta$ci.lb[1],2)` - `r round(Latency_meta$ci.ub[1], 2)`], $z = `r round(Latency_meta$z[1], 2)`$, $p = `r papaja::printp(Latency_meta$pval[1])`$) for 6–9 month-old monolingual infants. With the effect of age, the mean log latency estimate decreased significantly, with an estimated difference for the older group of `r round(Latency_meta$b[3],2)` (CI = [`r round(Latency_meta$ci.lb[3],2)` - `r round(Latency_meta$ci.ub[3], 2)`], $z = `r round(Latency_meta$z[3], 2)`$, $p = `r papaja::printp(Latency_meta$pval[3])`$); in other words, 12–15 month-old monolingual infants were faster than 6–9 month-old monolingual infants to fixate the congruent object. Bilingualism increased the mean log latency estimate by `r round(Latency_meta$b[2],2)` (CI = [`r round(Latency_meta$ci.lb[2],2)` - `r round(Latency_meta$ci.ub[2], 2)`], $z = `r round(Latency_meta$z[2], 2)`$, $p = `r papaja::printp(Latency_meta$pval[2])`$); in other words, the estimate for bilinguals suggested they might be slower than monolingual infants to fixate the congruent object, but this was non-significant. The interaction between bilingualism and age suggested a possible attenuation of this pattern for older 12–15 month-old bilingual versus monolingual infants, although again this did not reach statistical significance (estimate = `r round(Latency_meta$b[4],2)`, CI = [`r round(Latency_meta$ci.lb[4],2)` - `r round(Latency_meta$ci.ub[4], 2)`], $z = `r round(Latency_meta$z[4], 2)`$, $p = `r papaja::printp(Latency_meta$pval[4])`$). Pairwise comparisons revealed that, at the age of 12–15 months, there was no longer any evidence of a difference in target fixation latency between monolingual and bilingual infants (estimate = `r round(pairwise$Lb[2],2)`, $se = `r round(pairwise$se[2],2)`$, $z = `r round(pairwise$z[2], 2)`$, $p = `r papaja::printp(pairwise$pval[2])`$). A forest plot for this meta-analysis is shown in Figure 5.
...
ggplot(latency_forest_data,
  ...
  scale_colour_manual(name = "Language group", values = c("#bdbdbd", "#636363", "#000000"), breaks= c("bilingual", "monolingual")) +
  xlab("Lab") +
  ylab("Mean Log Latency to Congruent AOI") +
  theme(legend.position = "bottom")
```
