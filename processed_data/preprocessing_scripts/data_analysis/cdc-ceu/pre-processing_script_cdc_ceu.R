...
wd <- '~/Gaze following/data_analysis/cdc-ceu/'

# list files
files <- list.files(wd, pattern = '.txt')

# read in the data 
da <- read.delim(paste0(wd, files), na.string = '.')
...
# here is an example of what happens during the first trial of 'mb1':
da[1045:1065,]
# The VIDEO_NAME and FRAME_INDEX change while the TIMESTAMP is not distrubed, no clue what happens here
...
# Only the right eye is specified, therefore these measures are also taken as input
# for the left eye. In the analyses this doesn't matter as the signals are averaged
LEFT_EYE_X <- "RIGHT_GAZE_X"
LEFT_EYE_Y <- "RIGHT_GAZE_Y"
...
for(pp in unique(d$PP_NAME)){
  ...
  ## select the last trial to 'fix' the weird VIDEO orders
  index <- rle(pp_data$TRIAL_INDEX)
  ii <- cumsum(index$len)
  
  trial_selection <- numeric()
  for(i in sort(unique(index$values))){
    tr <- tail(which(index$values == i), 1)
    if(tr == 1){
      trial_selection <- c(trial_selection, 1:ii[tr])
    } else {
      trial_selection <- c(trial_selection, (ii[tr - 1] + 1):ii[tr])
    }
  }
  
  pp_data <- pp_data[trial_selection,]
  newd[[pp]] <- pp_data
  ## run gazepath
...
# save the necessary data of all trials
