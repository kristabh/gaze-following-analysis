...
wd <- '~/Gaze following/data_analysis/brookes-babylab/'

# list files
files <- list.files(paste0(wd, 'brookes-babylab raw data/'), pattern = '.tsv')

# read in the Tobii participant data and combine into 1 data frame
da <- numeric()
for(file in files){
  ppdata <- read.delim(paste0(wd, 'brookes-babylab raw data/', file), skip = 0)
  da <- rbind(da, ppdata)
}
...
SCREEN_RES_WIDTH_PX <- 1280
SCREEN_RES_HEIGHT_PX <- 720

SCREEN_WIDTH_mm <- 510
SCREEN_HEIGHT_MM <- 330
...
# Rename the trial 6 names for the participants in order 2
# In this subset either the naming of the file is 'wrong'
# Ord3_T6_L.avi, instead of Ord2_T6_L.avi or the wrong
# video was shown. To extract the right information was
# trials and order these video names are renamed

# participants in the order 2 condition
pps <- unique(d$PARTICIPANT[grep('Ord2', d$TRIAL_NAME)])
# rename
d$TRIAL_NAME <- as.character(d$TRIAL_NAME)
d$TRIAL_NAME[d$PARTICIPANT %in% pps & d$TRIAL_NAME == 'Ord3_T6_L.avi'] <- 'Ord2_T6_L.avi'
...
# scale factors do not match, this is odd
# Therefore I assume the actual offset on the left and right is 190
LO <- (SCREEN_RES_WIDTH_PX - 720*sfh)/2
RO <- (SCREEN_RES_WIDTH_PX - 720*sfh)/2

# offset factor
of <- (STIM_WIDTH_PX - 720 * sf) / 2
ofh <- (STIM_HEIGHT_PX - 576 * sfh) / 2

# calculate aoi's
aoif <- AOI_face * sfh + matrix(c(rep(of + LO, 4), rep(ofh + TO, 4)), 4, 2)
aoil <- AOI_left * sfh + matrix(c(rep(of + LO, 4), rep(ofh + TO, 4)), 4, 2)
aoir <- AOI_right * sfh + matrix(c(rep(of + LO, 4), rep(ofh + TO, 4)), 4, 2)
# add aoi's to plot
polygon(aoif)
polygon(aoil)
polygon(aoir)
