...
wd <- '~/Gaze following/data_analysis/UPF-Barcelona/'

# list files
files <- list.files(paste0(wd, 'UPF_Barcelona_RAWDATA-eyetracking/'), pattern = '.tsv')

# read in the Tobii participant data and combine into 1 data frame
da <- numeric()
# since the data of the 1st particpant contains weird values instead of gaze coordinates,
# the data of the first participant is left out this analysis
for(file in files[-1]){
  ppdata <- read.delim(paste0(wd, 'UPF_Barcelona_RAWDATA-eyetracking/', file), skip = 40)
  # store the pp id in every file
  ppdata$pp <- file
  da <- rbind(da, ppdata)
}
...
LEFT_EYE_X <- "GazePointXLeft"
LEFT_EYE_Y <- "GazePointYLeft"

RIGHT_EYE_X <- "GazePointXRight"
RIGHT_EYE_Y <- "GazePointYRight"
...
SCREEN_RES_WIDTH_PX <- 1920
SCREEN_RES_HEIGHT_PX <- 1080

SCREEN_WIDTH_mm <- 511
SCREEN_HEIGHT_MM <- 288
...
# Therefore I assume the actual offset on the left and right is 285
