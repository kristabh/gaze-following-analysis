...
wd <- '~/Gaze following/data_analysis/elp-georgetown/'

# read in the data 
da <- read.delim(paste0(wd, 'elp_georgetown_Data_Export.txt'))
...
# The eye-tracking details file mentions a samplerate of 300. The data, however, suggest is was actually 120
# therefore 120 is used in the analysis
SAMPLERATE <- 120
...
# There is a potential problem with the participants names
# that is, they do not follow a unique order and 3 are duplicates
# rle(d$PP_NAME)$val[duplicated(rle(d$PP_NAME)$val)]

# For now these participants are excluded, we can correct this later
d <- d[!d$PP_NAME %in% rle(d$PP_NAME)$val[duplicated(rle(d$PP_NAME)$val)],]
